...
def login_page():
    ...
    with row_1_col2:
        ...
        if Apelido != 'Suporte':
            st.session_state.Escola_L = st.selectbox('Escola', opcoes_escola, index=None, placeholder='Selecione uma escola...')
            
            if not st.session_state.Escola_L:
                st.error('O campo "Escola" é Obrigatorio.')
        else:
             st.session_state.Escola_L = 'Administrador'
    with row_1_col3:
        st.write('')
        ...
        if selected_login == 'Login':
            if Apelido == None or Password == None:
                ...
                if LoginDAO.authenticator(Apelido, Password):
                    ...
                    if st.session_state.Apelido_L == 'Suporte':
                        st.session_state.Suporte = True
                    else: 
                        st.session_state.Suporte = False
